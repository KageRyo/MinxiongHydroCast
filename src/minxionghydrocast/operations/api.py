...
PRODUCT_NOTICES = {
    ...
    "official_alert": (
        "Source product collected from Taiwan WRA. MinxiongHydroCast does not issue or replace "
        "official warnings."
    ),
    "official_observation": (
        "Observation collected from Taiwan WRA and validated by MinxiongHydroCast."
    ),
...
def metrics_payload(status: StatusResponse) -> str:
    ...
    lines = [
        "# HELP minxionghydrocast_ready Whether operational data passes readiness gates.",
        "# TYPE minxionghydrocast_ready gauge",
        f"minxionghydrocast_ready {ready}",
        "# HELP minxionghydrocast_last_attempt_success Whether the last collection succeeded.",
        "# TYPE minxionghydrocast_last_attempt_success gauge",
        f"minxionghydrocast_last_attempt_success {attempt_ok}",
        "# HELP minxionghydrocast_dataset_age_minutes Age of the latest dataset observation.",
        "# TYPE minxionghydrocast_dataset_age_minutes gauge",
    ]
    for name, details in sorted(status.datasets.items()):
        age = details.health.age_minutes
        if age is not None:
            lines.append(f'minxionghydrocast_dataset_age_minutes{{dataset="{name}"}} {age}')
    lines.extend(
        [
            "# HELP minxionghydrocast_dataset_state Current dataset health state.",
            "# TYPE minxionghydrocast_dataset_state gauge",
        ]
    )
    for name, details in sorted(status.datasets.items()):
        state = details.health.state
        lines.append(
            f'minxionghydrocast_dataset_state{{dataset="{name}",state="{state}"}} 1'
        )
    lines.extend(
        [
            "# HELP minxionghydrocast_dataset_source_kind Active dataset source kind.",
            "# TYPE minxionghydrocast_dataset_source_kind gauge",
        ]
    )
    for name, details in sorted(status.datasets.items()):
        if details.source is not None:
            lines.append(
                "minxionghydrocast_dataset_source_kind"
                f'{{dataset="{name}",source_kind="{details.source.source_kind}"}} 1'
            )
    lines.extend(
        [
            "# HELP minxionghydrocast_dataset_source_outcome Active source collection outcome.",
            "# TYPE minxionghydrocast_dataset_source_outcome gauge",
        ]
    )
    for name, details in sorted(status.datasets.items()):
        if details.source is not None:
...
def shadow_metrics_payload(report: ShadowResponse) -> str:
    ...
    return (
        "# HELP minxionghydrocast_shadow_gate_passed Whether shadow criteria passed.\n"
        "# TYPE minxionghydrocast_shadow_gate_passed gauge\n"
        f"minxionghydrocast_shadow_gate_passed {passed}\n"
        "# HELP minxionghydrocast_notification_allowed Whether notifications may be enabled.\n"
        "# TYPE minxionghydrocast_notification_allowed gauge\n"
        f"minxionghydrocast_notification_allowed {allowed}\n"
    )
...
<head>
  ...
  <title>MinxiongHydroCast Operations</title>
  <style>
    :root {
      color-scheme: light;
...
<body>
  <header>
    <div>
      <h1>MinxiongHydroCast Operations</h1>
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Serve the MinxiongHydroCast observation API and operator view.",
    )
    ...
    print(f"[OK] Serving MinxiongHydroCast operations at http://{address}:{port}")
    try:
