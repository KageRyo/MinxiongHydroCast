...
`mhc source cwa-event-plan --download`. It validates cadence, grid consistency, CRS, units,
...
Do not read `CWA_API_KEY` inside the adapter. Downloading belongs in `mhc source cwa-download` or
`mhc source cwa-event-plan --download`; the adapter consumes already downloaded local files and
redacted manifests.
